...
// Demonstration file in 3D with fluid cells only and periodic boundary conditions
...
int main()
{
	...
	int hr = ReadData("../test/lbm_sim3D_f0.dat", f0, sizeof(real), SIZE_3D_X*SIZE_3D_Y*SIZE_3D_Z*19);
	if (hr < 0) {
		fprintf(stderr, "File containing distribution function data not found, exiting...\n");
		return hr;
	}

	// allocate and initialize start field consisting of fluid cells only
	...
	for (z = 0; z < SIZE_3D_Z; z++)
	{
		for (y = 0; y < SIZE_3D_Y; y++)
		{
			for (x = 0; x < SIZE_3D_X; x++)
			{
				...
				DFD3Q19_Init(CT_FLUID, &f0[19*((x*SIZE_3D_Y + y)*SIZE_3D_Z + z)], LBM3DField_Get(&startfield, x, y, z));
			}
		}
	}
